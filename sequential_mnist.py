...
pd = {}
pd['problem'] = 'MNIST'
pd['architecture'] = 'FDN'  # 'GRU'  'WaveletGRU' 'FastFoodGRU'
pd['num_channels'] = [30, 30, 30]
pd['epochs'] = 10
pd['lr'] = 5e-3
...
for current_run_pd in pd_lst:
    ...
    optimizer = torch.optim.Adam(FDN.parameters(), current_run_pd['lr'])
    ...
    def train_test_loop(x, y, iteration_no, e_no, train):
        ...
        if iteration_no % 50 == 0:
            print('e', e_no, 'step', iteration_no, 'loss', cpu_loss, 'acc', acc, 'train', train)
        return cpu_loss, acc
